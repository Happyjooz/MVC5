...
namespace WebApplication1.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                {
                    ...
                    {
                        var uri = new Uri("http://localhost/GalaxyService/GalaxyAPI.svc/login?login=%22hello%22&password=%22world%22");
        ...
        // POST: /Account/Login2
        [AllowAnonymous]
        public async Task<string> Login2(string login, string password)
        {
            // Ceci ne comptabilise pas les échecs de connexion pour le verrouillage du compte
            // Pour que les échecs de mot de passe déclenchent le verrouillage du compte, utilisez shouldLockout: true
            var result = await SignInManager.PasswordSignInAsync(login, password, false, shouldLockout: false);
            switch (result)
            {
                case SignInStatus.Success:
                    {
        ...
        //
        // POST: /Account/LogOff2
        public string LogOff2()
        {
            AuthenticationManager.SignOut(DefaultAuthenticationTypes.ApplicationCookie);
            return "LogOff";
        }
